...
The Slack widget takes many parameters, like, for example, the channel ID it will broadcast to (send the message to) and the actual message. For right now, we'll set the message to "Hello, World!" Classic. 😼 As a side note, we can do that in Slack to get the specific channel ID we want to send our message to. By right-clicking the specific channel and navigating a little bit, we can find the channel ID. By clicking "Test," we can test to see if our Slack widget is working as expected. 
